...
// --- 前向声明 ---
template <typename Signature> class function; // 主模板声明
...
private:
  ...
  template <typename F> struct function_derived : function_base {
    ...
    R invoke(Args... args) const override {
      // 移除判空检查，因为大多数可调用对象没有operator!
      return std::invoke(callable, std::forward<Args>(args)...);
    }

    // (可选) 实现 target_type 和 target
    // const std::type_info& target_type() const noexcept override { return
    // typeid(F); } void* target_ptr() noexcept override { return
    // std::addressof(callable); } const void* target_ptr() const noexcept
    // override { return std::addressof(callable); }
  };
...
public:
  ...
  function(function &&other) noexcept {
    ...
    // SBO 版本：移动指针或缓冲区内容，并将 other 的 manager/invoker
    // 置空。通常是 noexcept。
  }
  ...
  ~function() {
    ...
    // SBO 版本：如果使用了堆内存 (uses_heap_ = true)，需要调用 manager
    // 函数销毁堆对象并释放内存。如果对象在 buffer 中，需要调用 manager 销毁它
    // (调用析构函数)。
  }
  ...
  R operator()(Args... args) const {
    ...
    // storage_ptr() 需要根据 uses_heap_ 返回 buffer 地址或 ptr
  ...
  // 如果匹配，返回指向存储对象的指针 (通过 base_ptr_->target_ptr() 或 SBO 的
  // manager)。 否则返回 nullptr。 const T* const_this = static_cast<const
  // function*>(this)->target<T>(); return const_cast<T*>(const_this);
  // }
  ...
  //    // return static_cast<const T*>(base_ptr_->target_ptr()); // 假设有
  //    target_ptr return nullptr; // Placeholder
  // }
  // return nullptr;
  // }
